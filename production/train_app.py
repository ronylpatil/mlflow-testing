...
def train(n_est: int, crit: str, maxd: int, mss: int, msl: int) -> tuple : 
     ...
     outs = train_model(X_train, Y, n_estimators = n_est, criterion = crit, max_depth = maxd, min_samples_split = mss, min_samples_leaf = msl, random_state = parameters['seed'] , yaml_file_obj = params)
...
def new_experiment(exp_name: str, exp_des: str, run_desc: str, n_est: int, crit: str, maxd: int, mss: int, msl: int) -> float : 
     outs, filename, model_dir, params = train(n_est, crit, maxd, mss, msl)
     ...
     with mlflow.start_run(description = run_desc) : 
          mlflow.log_params({"n_estimator": outs['params']['n_estimator'], "criterion": outs['params']['criterion'], 
                              "max_depth": outs['params']['max_depth'], "seed": outs['params']['seed']})
...
def existing_exp(exp_name, run_desc, n_est, crit, maxd, mss, msl) -> float :
     outs, filename, model_dir, params = train(n_est, crit, maxd, mss, msl)
